...
impl<'a> Parser<'a> {
    ...
    pub fn parse_identifier(&mut self) -> Result<Ident, ParserError> {
        let m = self.start();
        match self.next_token() {
            Token::Word(w) => ret!(Ok(w.to_ident())
                => via self.complete(m, SK::IDENT, ..)),
            Token::SingleQuotedString(s) => Ok(Ident::with_quote('\'', s)), // via?
    ...
    pub fn parse_query(&mut self) -> Result<Query, ParserError> {
        let m = self.start();
        let with = if let Some(m) = self.start_if(|p| p.parse_keyword(Keyword::WITH)) {
        ...
        if !self.parse_keyword(Keyword::INSERT) {
            ...
            let order_by = if let Some(m) =
                self.start_if(|p| p.parse_keywords(&[Keyword::ORDER, Keyword::BY]))
            {
            ...
            let limit = if let Some(_m) = self.start_if(|p| p.parse_keyword(Keyword::LIMIT)) {
                self.parse_limit()? // TBD
            } else {
                None
            };

            let offset = if let Some(_m) = self.start_if(|p| p.parse_keyword(Keyword::OFFSET)) {
                Some(self.parse_offset()?) // TBD
            } else {
                None
            };

            let fetch = if let Some(_m) = self.start_if(|p| p.parse_keyword(Keyword::FETCH)) {
                Some(self.parse_fetch()?) // TBD
            } else {
                None
            };

            ret!(Ok(Query { with, body, limit, order_by, offset, fetch })
                => via self.complete(m, SK::QUERY, ..))
        } else {
            let insert = self.parse_insert()?;
            ret!(Ok(Query { with, body: SetExpr::Insert(insert), limit: None, order_by: vec![],
                offset: None,
                fetch: None }) => via self.complete(m, SK::QUERY, ..))
        }
    }
    ...
    fn parse_query_body(&mut self, precedence: u8) -> Result<SetExpr, ParserError> {
        ...
        let mut expr =
            if let Some(m) = self.start_if(|parser| parser.parse_keyword(Keyword::SELECT)) {
    ...
    pub fn parse_select(&mut self) -> Result<Select, ParserError> {
        ...
        let from = if let Some(m) = self.start_if(|parser| parser.parse_keyword(Keyword::FROM)) {
        ...
        let selection =
            if let Some(m) = self.start_if(|parser| parser.parse_keyword(Keyword::WHERE)) {
    ...
    pub fn parse_table_factor(&mut self) -> Result<TableFactor, ParserError> {
        if self.parse_keyword(Keyword::LATERAL) {
            ...
            match self.parse_derived_table_factor(NotLateral) {
                ...
                Ok(table_factor) => return Ok::<TableFactor, ParserError>(table_factor),
                Err(_) => {
    ...
    fn parse_join_constraint(&mut self, natural: bool) -> Result<JoinConstraint, ParserError> {
        ...
        } else if let Some(m) = self.start_if(|parser| parser.parse_keyword(Keyword::ON)) {
            let constraint = self.parse_expr()?;
            ret!(Ok(JoinConstraint::On(constraint))
                => via self.complete(m, SK::JoinConstraint__On, ..))
        } else if let Some(m) = self.start_if(|parser| parser.parse_keyword(Keyword::USING)) {
            let columns = self.parse_parenthesized_column_list(Mandatory)?;
